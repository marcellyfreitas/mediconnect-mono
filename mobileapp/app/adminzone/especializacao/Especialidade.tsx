...
export default function EspecialidadeCreateScreen() {
	...
	const { control, handleSubmit, formState: { errors } } = useForm<InnerFormData>({
		...
		defaultValues: {
			...
			description: (routeParams.description ?? '') as string,
		},
	});
	...
	const onSubmit = async (data: InnerFormData) => {
		try {
			...
			if (!routeParams.editable) {
				await service.addFromAdminAsync({ ...data });
			}

			if (routeParams.editable) {
				await service.updateFromAdminAsync(Number(routeParams.id), { ...data });
			}
			...
			router.dismissAll();
			router.replace('/adminzone/especializacao');
		} catch (error: any) {
