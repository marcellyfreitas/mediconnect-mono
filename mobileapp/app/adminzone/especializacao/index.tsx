...
import { EspecialidadeService } from '@/services/restrict/EspecialidadeService';
import { useActionSheet } from '@expo/react-native-action-sheet';
import Toast from 'react-native-root-toast';
import { colors } from '@/utils/constants';

const { client } = HttpClient(USER_ACCESS_TOKEN_NAME);
const service = new EspecialidadeService(client);

const EspecialidadeListScreen = () => {
	...
	function handleEdit(item: any) {
		router.push({
			pathname: '/adminzone/especializacao/Especialidade',
	...
	return (
		<ThemedView className="relative flex-1 w-full p-5">
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<CardCrud item={item} onDelete={onPressDelete} onEdit={handleEdit}>
							<Text className="text-lg font-semibold uppercase text-slate-600">{item.name}</Text>
							<Text className="text-sm font-semibold text-slate-600">{item.description}</Text>
						</CardCrud>
					)}
			...
			<Button
				onPress={() => router.push('/adminzone/especializacao/Especialidade')}
				circular
				color="primary"
...
export default EspecialidadeListScreen;
